...
impl CodeSmellDensityStats {
    pub fn calculate_smell_density(&mut self, code: &str) -> f64 {
        ...
        if self.has_long_functions(code) {
            smells.push(SmellType { name: "Long Functions".to_string(), count: 1, severity: 0.8 });
        }
        
        if self.has_duplicate_code(code) {
            smells.push(SmellType { name: "Duplicate Code".to_string(), count: 1, severity: 0.7 });
        }
        
        if self.has_deep_nesting(code) {
            smells.push(SmellType { name: "Deep Nesting".to_string(), count: 1, severity: 0.9 });
        }
        
        if self.has_magic_numbers(code) {
            smells.push(SmellType { name: "Magic Numbers".to_string(), count: 1, severity: 0.5 });
        }
        
        if self.has_dead_code(code) {
            smells.push(SmellType { name: "Dead Code".to_string(), count: 1, severity: 0.6 });
        }
